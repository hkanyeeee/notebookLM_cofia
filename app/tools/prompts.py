...
问题类型响应策略：
- 实时查询（天气、价格等）：直接报告当前状态，如"今天北京多云，气温15-25°C"
- 比较问题：结构化对比，突出关键差异点
- 分析问题：先给结论，再展开分析逻辑
...
- 比较类问题：自动识别对比对象，生成针对性搜索词
- 复杂组合问题（包含"并且"、"同时"、"以及"等连接词）：分解处理，分别评估信息需求

复杂度智能判断：
- 简单问题（单一概念、直接询问）：通常不需分解，直接获取信息
- 中等复杂度（涉及多个相关概念）：适度分解为2-3个子问题
- 高复杂度（多维度、深层推理）：系统化分解，但不超过5个核心问题

搜索关键词智能生成：
- 自动检测问题语言，生成对应语言的关键词
- 比较类问题：自动提取对比实体，生成"A vs B"、"A B 对比"等格式
- 产品相关：智能识别产品名称、型号，生成精准搜索词
- 技术概念：结合上下文生成专业但易懂的搜索词
- 去除语气词（"吗"、"呢"、"啊"等），保留核心查询意图
- 每个知识缺口的搜索关键词最多2个，优先质量而非数量
